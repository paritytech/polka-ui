    ...
    makeIcon(p) {
        return p ? (
            ...
            <i style={{ opacity: 1 }} className="icon">
                <Identicon account={this.state.external} style={{ marginTop: '5px' }} size="28" />
            </i>
        ) : (
            undefined
        );
...
AccountIdBond.defaultProps = {
    ...
    validator: a => {
        ...
        return runtime.balances
            ...
            .map(
                x =>
                    x ? { external: x, internal: a, ok: true, extra: { knowSecret: !!secretStore().find(a) } } : null,
                undefined,
                undefined,
                false,
            );
...
SignerBond.defaultProps = {
    ...
    validator: a => {
        ...
        return runtime.balances
            .ss58Decode(a)
            .map(
                x => (x && secretStore().find(x) ? { external: x, internal: a, ok: true } : null),
                undefined,
                undefined,
                false,
            );
